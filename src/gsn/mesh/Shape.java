...
public class Shape extends DummyObject
{
	...
	*/
	...
	public static Shape rect(float w, float h)
	{
		Shape out = new Shape();
	...
	public static Shape arc(float radStart, float radEnd, float inner, float outer, int steps)
	{
		float TWO_PI = (float)Math.PI*2.0f;
		Shape out = new Shape();

		// Make sure inner radius is the smaller one
		if(inner > outer)
		{
			float t = inner;
			inner = outer;
		...
		// Make sure we only work with positive values
		while(radStart < 0.0f)
		{
			radStart += TWO_PI;
			radEnd += TWO_PI;
		...
		if(inner > 0.0f)
		{
			...
			{
				Vector p0 = Vector.polar2cart(radStart*(da*s) + radEnd*(1.0f-(da*s)), outer);
				out.addVertex(p0);
			}
			for(int s = steps-1; s >= 0; s--)
			{
				Vector p1 = Vector.polar2cart(radStart*(da*s) + radEnd*(1.0f-(da*s)), inner);
				out.addVertex(p1);
			}			
			//endShape();
		}
		else 
		{
			...
			out.addVertex(new Vector(0.0f, 0.0f));
			for(int s = 0; s < steps; s++)
			{
				Vector p0 = Vector.polar2cart(radStart*(da*s) + radEnd*(1.0f-(da*s)), outer);
				out.addVertex(p0);
